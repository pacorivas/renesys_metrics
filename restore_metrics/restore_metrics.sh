...
daemon_function() {
  while true; do
    ...
    do
      ...
      mysql -sN --host=${MYSQL_HOST} --port=${MYSQL_PORT} \
                --user=${MYSQL_USER} --password=${MYSQL_PASS} \
                -e "REPLACE INTO cuadro_mandos.restore_metrics(file,table,restoration_date,info,restore_date_init,restore_date_end) VALUES ('${FILE_NAME}',${TABLE_NAME},${RESTORE_DATE},'campo info','${INIT_DATE}','2000-01-01 00:00:00')" 2> /dev/null
      ...
      mysql -sN --host=${MYSQL_HOST} --port=${MYSQL_PORT} \
                --user=${MYSQL_USER} --password=${MYSQL_PASS} \
                -e "REPLACE INTO cuadro_mandos.restore_metrics(file,table,restoration_date,info,restore_date_init,restore_date_end) VALUES ('${FILE_NAME}',${TABLE_NAME},${RESTORE_DATE},'campo info','${INIT_DATE}','${END_DATE}')" 2> /dev/null
